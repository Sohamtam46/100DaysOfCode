...
def new_word_generate():
    ...
    lang_words_df =pd.DataFrame(lang_words)
    lang_words_dict = lang_words_df.to_dict(orient="records")
...
title_text = canvas.create_text(400,150,text=f"{LANG1} Learning Flash Cards",fill="black",font=(FONT_NAME,40,"italic"))
word_text = canvas.create_text(400,263,text=f"Hit Green to Start",fill="black",font=(FONT_NAME,60,"bold"))
canvas.grid(column=0,row=0,columnspan=2)

right_button_image = PhotoImage(file="./images/right.png")
right_button = Button(image=right_button_image, command=new_word_generate, highlightthickness=0)
